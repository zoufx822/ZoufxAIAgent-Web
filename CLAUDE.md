...
- **通知**: 顶部胶囊 top-toast（store `topToast` 驱动，失败/停止时轻提示）
...
components/              # React 组件
  ...
  top-toast.tsx          # 顶部胶囊轻提示（store.topToast 驱动）
  ui/                    # tooltip / textarea / stream-markdown（揭示引擎）

lib/                     # 核心逻辑
  store.ts               # Zustand 主 store（锚点元数据 + 消息 + UI/情绪态 + prefill/pendingTurn/topToast）
  chat-stream.ts         # SSE 流解析（streamChat；含 turn 事件 + watchdog→onIdleTimeout）
  pending-poll.ts        # consumeStream 在建轮轮询（等服务端落库回复替换「生成中」占位）
  api.ts                 # 锚点 / 消息 / pending / stop / context / Hot Memory REST 客户端
...
hooks/                   # 自定义 React Hooks
  ...
  use-anchor-messages.ts # 切锚拉取窗口消息 + 检查在建轮（pending）补「问题+生成中」轮询
...
**Anchor**: 对话锚点，含标题和创建时间；messages 不内嵌，按需从后端拉取。

**Store**: Zustand 主 store，持有锚点元数据、消息、UI/情绪态；仅 userId/anchors/currentAnchorId 持久化。
...
- **消息不持久化**：切锚时由 `useAnchorMessages` 从后端按需拉取窗口消息
...
- 处理事件：`turn_started`（首帧 turnId）、`anchor_created`、`thinking`、`content`、`tool_call`、`tool_result`、`mood`、`error`
- **无数据 watchdog（consumeStream 语义）**: 每次 `reader.read()` 与 90s 定时器 race；超时**不当失败**——cancel 连接后触发 `onIdleTimeout`，交上层标「生成中」+ 轮询 `loadMessages`（生成大概率仍在服务端跑）。阈值须 > 后端单次工具调用最坏阻塞 ~60s
- 尾部数据缓冲处理：规范化未完成的最后消息
- 停止走独立端点 `POST /ai/chat/stop`；`AbortSignal` 仅用于关闭客户端连接（断连≠停止生成）
...
- 顶部胶囊 `.top-toast`（globals.css）提供轻提示，由 `TopToast` 组件按 `store.topToast` 渲染

## 后端集成

前端期望在 `${NEXT_PUBLIC_API_URL}/ai/chat` 处有 POST 端点：

- 接收 JSON：`{ prompt, anchorId, thinking: {enabled, effort}, userId }`（anchorId 为空时后端懒创建锚点；锚点摘要压缩改由后端定时扫描，前端不再传 prevAnchorId）
- 返回 SSE，包含事件：
  - `turn_started`: 本轮 turnId（首帧，早于 `anchor_created`）——前端存下供停止/轮询定位
  - `anchor_created`: 懒建锚点返回的新 anchorId（新对话首条消息）
  - `thinking`: 扩展思考内容（可选）
  - `content`（默认事件）: 响应文本块
  - `tool_call`: JSON `{ tool, toolDisplay, query }`
  - `tool_result`: JSON `{ tool, toolDisplay, count, resultPreview }`
  - `mood`: JSON `{ keyword }`，后端从 content 流就地剥离 `⟦mood:KEYWORD⟧` 标记后独立发送，一轮 1~N 次（回复开头必有一个「第一反应」情绪，情绪转折处再追加；无独立瞬时分类事件）
  - `error`: 包含 `error.message` 或 `message` 字段的 JSON 错误
- **consumeStream**：停止走 `POST /ai/chat/stop {turnId}→{stopped}`；在建轮问题查 `GET /ai/anchors/{id}/pending`；关页/断连不掐生成，后端跑完落库，前端轮询 `loadMessages` 补显
- 锚点 CRUD / 消息 / pending / context / Hot Memory 走 `lib/api.ts` 的 REST 端点（`/ai/anchors*`、`/ai/memory/hot`）
