...
- 接收 JSON：`{ prompt, anchorId, prevAnchorId, thinking: {enabled, effort}, userId, regenerate? }`（anchorId 为空时后端懒创建锚点，首条事件 `anchor_created` 回传新 id）
  - `regenerate`（可选 boolean）：`true` = 重试同一轮（前端对「最后一条」出错消息点「重试」时发送）。后端应**回滚该 anchor 最后一轮 assistant 输出后就地再生成**，而非把 prompt 当新一轮追加——避免重复 user 轮污染记忆窗口。前端 UI 已把重试门控在最后一条，故「回滚最后一轮」无歧义；省略或 `false` 即普通新轮。
- 返回 SSE，包含事件：
  - `thinking`: 扩展思考内容（可选）
  - `content`（默认事件）: 响应文本块
